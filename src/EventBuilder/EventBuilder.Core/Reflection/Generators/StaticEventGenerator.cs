...
namespace EventBuilder.Core.Reflection.Generators
{
    internal class StaticEventGenerator : EventGeneratorBase
    {
        ...
        private IEnumerable<ClassDeclarationSyntax> GenerateClasses(IEnumerable<(ITypeDefinition typeDefinition, IEnumerable<IEvent> events)> declarations)
        {
            return declarations.OrderBy(x => x.typeDefinition.Name).Select(x => GenerateStaticClass(x.typeDefinition, x.events));
        }

        private ClassDeclarationSyntax GenerateStaticClass(ITypeDefinition typeDefinition, IEnumerable<IEvent> events)
        {
            // Produces:
